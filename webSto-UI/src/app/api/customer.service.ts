...
export class CustomerService implements RestService<CustomerResource> {
  ...
  getAllCustomers(): Observable<DocumentCollection<CustomerResource>> {
    return new Observable<DocumentCollection<CustomerResource>>( (subscriber) => {
      this.http.get<IDataCollection>(`${environment.getApiUrl()}external/customers/list`).subscribe( (data) => {
